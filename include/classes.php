...
class mf_media
{
	...
	function init()
	{
		...
		register_post_type($this->post_type_allowed, array(
			'labels' => array(
				...
				'all_items' => __("List", 'lang_media'),
				'edit_item' => __("Edit", 'lang_media'),
				'view_item' => __("View", 'lang_media'),
				'add_new_item' => __("Add New", 'lang_media'),
			),
			'public' => false,
			'show_ui' => true,
			'show_in_menu' => false,
	...
	function upload_mimes($existing_mimes = [])
	{
		...
		$result = $obj_base->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s", $this->post_type_allowed, 'publish'));
...
<?php
	...
	function filter_is_file_used($arr_used)
	{
		...
		// Content
		####################
		$result = $wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE (post_content LIKE %s OR post_content LIKE %s OR post_content LIKE %s OR post_content LIKE %s)", "%".$arr_used['file_url']."%", "%".$arr_used['file_thumb_url']."%", "%\"wp-image-".$arr_used['id']."\"%", "%\"id\"\:".$arr_used['id'].",%"));
		$rows = $wpdb->num_rows;
